...
export async function fetchWallet(address) {
  return getJSON(`/api/wallet?address=${encodeURIComponent(address)}`);
}

export async function fetchWalletTransfers(address) {
  return getJSON(`/api/wallet/transfers?address=${encodeURIComponent(address)}`);
}

export async function fetchNft() {
  return getJSON('/api/nft');
}
